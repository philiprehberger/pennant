...
const CONFIG = {
    ...
    paths: {
        ...
        basePath: (process.env.SERVER_BASE_PATH || process.env.SERVER_DEST_PATH || '/var/www/pennant').trim(),
...
async function deploy(options = {}) {
    ...
    try {
        ...
        await execSSH(ssh, 'sudo supervisorctl restart pennant-horizon:*', { ignoreError: true });
        log('✅', 'Queue workers restarted');

        log('♻️', ' Restarting Reverb WebSocket server...');
        await execSSH(ssh, 'sudo supervisorctl restart pennant-reverb', { ignoreError: true });
        ...
        if (!options.skipVerify) {
            ...
            let healthOk = false;
            const healthUrl = `https://${process.env.MARKETING_DOMAIN || 'api.pennant.dcsuniverse.com'}/health`;
        ...
        log('🌐', 'Site: https://api.pennant.dcsuniverse.com');
